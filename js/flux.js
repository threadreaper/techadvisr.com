...
function openNav() {
	qS(".hamburger-input").setAttribute('onclick','closeNav()')
	qS(".sidenav").style.width = "250px";
	qS(".body").style.marginLeft = "250px";
	document.body.style.backgroundColor = "rgba(0,0,0,0.6)";
}
  
function closeNav() {
	qS(".hamburger-input").setAttribute('onclick','openNav()')
	qS(".sidenav").style.width = "0";
	qS(".body").style.marginLeft = "0";
...
class Solver {
	...
	css(filters) {
		...
		return `filter: invert(${fmt(0)}%) sepia(${fmt(1)}%) saturate(${fmt(
			...
			4
		)}%) contrast(${fmt(5)}%) drop-shadow(1px 1px 2px var(--secondary));`;
	}
}
	...
	setTimeout(function () {
		...
		for (i = 0; i < icons.length; i += 1) {
			if (icons[i].classList.contains("filter-primary") == true) {
				...
				icons[i].classList.contains("filter-secondary") == true
			) {
				icons[i].addEventListener("mouseover", function () {
