...
document.addEventListener('mouseleave', function(event) {    
    var exitIntent = getComputedStyle(document.documentElement).getPropertyValue('--exit-intent').trim();
...
class Solver {
    ...
    css(filters) {
        ...
        return `filter: invert(${fmt(0)}%) sepia(${fmt(1)}%) saturate(${fmt(2)}%) hue-rotate(${fmt(3, 3.6)}deg) brightness(${fmt(4)}%) contrast(${fmt(5)}%) drop-shadow(1px 1px 2px var(--secondary));`;
    }
}
